---
name: context-swarm
description: Use when the user asks for an overview of an unfamiliar codebase, when starting work on a project the agent has never touched, when a goal requires understanding architecture before editing, or after a long break from the repo. Deploys dcc-specialist and livespec-specialist as parallel Task subagents; each produces a notion file under .claude/notions/ so the main agent reasons over digests instead of raw source.
---
...
- When the user asks "give me an overview", "what is this codebase", "where should I look first", or any equivalent.
