...
def main():
    ...
    # Code JSONL transcripts. No tmux injection — the old slash-scrape
    # path was removed in favour of pure file reads.
    try:
...
def _refresh_usage_state(project_dir: str) -> None:
    ...
    """
    try:
        from jig.engines import usage_local, usage_state
    except ImportError:
        return
...
def _emit_usage_block(project_dir: str) -> None:
    ...
    """
    # Try the installed jig package first (normal MCP runtime). When the
    ...
    try:
        from jig.engines.usage_state import format_traverse_block, read
        state = read()
        if not state:
            return
...
def _fallback_read_state() -> dict:
    """Read usage_state.json without depending on the jig package."""
    override = os.environ.get("JIG_USAGE_DIR")
    base = Path(override) if override else Path.home() / ".local" / "share" / "jig" / "usage"
